...
import { getResendFromAddress, isResendConfigured, normalizeEmail, sendEmail } from "@/lib/email/resend";
import { googleReviewUrlForBusiness } from "@/lib/sms/review-link";
import type { GeoGridPoint } from "@/audit/types";

export interface SendEmailReviewRequestsInput {
  userId: string;
...
export interface SendEmailReviewRequestsResult {
  sent: number;
  failed: number;
  skipped: number;
  simulated: boolean;
...
function hasReachableEmail(customer: CustomerRecord): boolean {
...
async function resolveCustomers(input: {
  ...
  if (input.customerIds && input.customerIds.length > 0) {
    ...
    return {
      customers: customers.filter(hasReachableEmail),
  ...
  const emailPool = pool.filter(hasReachableEmail);
  ...
  if (!input.focusKeyword?.trim()) {
    return {
      customers: emailPool.slice(0, input.batchSize),
      keywordFilterApplied: false,
      geoFilterApplied: false,
    };
  }

  const keywordSelected = selectCustomersForCampaign(emailPool, input.focusKeyword, input.batchSize);
  return {
...
export async function sendEmailReviewRequests(
  input: SendEmailReviewRequestsInput
): Promise<SendEmailReviewRequestsResult> {
  ...
  const result: SendEmailReviewRequestsResult = {
    sent: 0,
    failed: 0,
    skipped: 0,
    simulated: !isResendConfigured() && !input.dryRun,
  ...
  const fromAddress = getResendFromAddress(input.business.name) ?? undefined;
  ...
  for (const customer of customers) {
    ...
    const unsubscribeUrl = buildUnsubscribeUrl(customer.id, businessId);
    ...
    if (!isResendConfigured()) {
      const logMessage = input.serviceRole ? logEmailMessageAdmin : logEmailMessage;
